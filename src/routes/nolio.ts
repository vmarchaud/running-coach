...
router.get("/connect", async (c) => {
  return withFlow("nolio_auth_gate", async () => {
...
router.get("/callback", async (c) => {
  ...
  try {
    return await withFlow("nolio_auth_gate", async () => {
      if (error || !code) {
    ...
    const message = err instanceof Error ? err.message : "missing_code";
    return c.redirect(`/?nolioError=${encodeURIComponent(message)}`);
  }
});
