...
router.get("/callback", async (c) => {
  ...
  try {
    const userId = await withFlow("nolio_auth_session_gate", async (flow) => {
      if (error || !code) {
        const err = new Error(error ?? "missing_code");
        flow.fail(err);
        throw err;
      }
    ...
    return c.redirect(`/?nolioError=${encodeURIComponent("auth_failed")}`);
  }
});
