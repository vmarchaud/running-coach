...
router.get("/connect", flowMiddleware('nolio_auth_and_session'),  async (c) => {
...
router.get("/callback", flowMiddleware('nolio_auth_and_session'),  async (c) => {
...
router.delete("/disconnect", flowMiddleware('nolio_auth_and_session'),  async (c) => {
  const userId = c.get("userId");
  const db = createDb(c.env.DB);
  await db.delete(nolioTokens).where(eq(nolioTokens.userId, userId));
