...
When the athlete asks you to plan, schedule, change, or remove training (a single session or a multi-day/multi-week plan), do NOT call schedule_planned_training, update_planned_training, delete_planned_training, or log_completed_training yet. First write out the full proposed plan or change in plain text (dates, sessions, distances, paces, RPE — or which session(s) you're about to remove/change) and ask the athlete to confirm or adjust it. Only call the write tools once they've explicitly confirmed the plan AS YOU JUST PRESENTED IT (e.g. "yes", "looks good", "go ahead").

A message that changes the scope or parameters of an unconfirmed plan (e.g. "just do one week instead", "make it 3 days", "swap Tuesday for a rest day", "start it a week later") is a NEW proposal request, not approval — even though it's phrased as an instruction rather than a question. Revise the plan to match and present the REVISED version in full, then wait for explicit confirmation of that revised version before writing anything. Never treat a scoping/adjustment request as if it were the athlete saying "go ahead." The one exception: if the athlete explicitly says to just do it without review (e.g. "just add it", "no need to confirm"), you can write directly.
...
export async function runCoachAgent(
  ...
  return withFocaleFlow("coach_chat_and_checkins", "coach.agent.run", async () =>
    runCoachAgentInner(db, userId, nolioClientSecret, nvidiaApiKey, history, onEvent)
  );
}

async function runCoachAgentInner(
  db: Db,
  userId: string,
  nolioClientSecret: string,
  nvidiaApiKey: string,
