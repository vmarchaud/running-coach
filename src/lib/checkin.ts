...
export async function runScheduledCheckins(env: CheckinEnv): Promise<void> {
  return withFlow("scheduled_checkins", async () => {
    ...
    // Only surface a failure to the flow if every attempted check-in failed—
    // a partial failure is still a successful run overall.
